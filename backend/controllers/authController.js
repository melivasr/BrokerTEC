export async function register(req, res) {
  ...
  try {
    ...
    // Insertar usuario
    await queryDB(
      `INSERT INTO Usuario (alias, nombre, direccion, pais_origen, telefono, correo, contraseña_hash, rol, estado)
       VALUES (@alias, @nombre, @direccion, @pais_origen, @telefono, @correo, @contraseña_hash, @rol, 'Activo')`,
      { alias, nombre, direccion, pais_origen, telefono, correo, contraseña_hash, rol }
    );
...
export async function login(req, res) {
  ...
  try {
    ...
    res.json({ token, user: { alias: user.alias, rol: user.rol } });
  } catch (error) {
    console.error(error);
